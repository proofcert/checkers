...
prove((A,B),Ind,Label,Univ,Inst,_,AllLits,UnExp,Sleep,AllLab,
      NonGrd,Free,Forbid) :- !,
        copy_term((Label,Univ,Free),(LabelB,UnivB,Free)),
        prove(A,[l|Ind],Label,Univ,Inst,AllLits,AllLits,[(UnivB:Inst:LabelB:B:[r|Ind])|UnExp],
...
prove((A;B),Ind,Label,Univ,Inst,_,AllLits,UnExp,Sleep,AllLab,
	...
	copy_term((Label,Univ,Free,Ind),(LabelA,UnivA,Free,IndA)),
        copy_term((Label,Univ,Free),(LabelB,UnivB,Free,IndB)),
        extend_inst(Univ,Inst,NewInst),
        make_forbid(Univ,Inst,AddForbid),
        append(AddForbid,Forbid,NewForbid),
        append(Sleep,[(UnivA:NewInst:LabelA:(A;B):IndA)-Univ],SleepA),
        append(Sleep,[(UnivB:NewInst:LabelB:(A;B):IndB)-Univ],SleepB),
...
prove(box Fml,Ind,Label,Univ,Inst,_,AllLits,UnExp,Sleep,AllLab,
      NonGrd,Free,Forbid) :- !,
	prove(Fml,Ind,[((X,_,Ind),_)|Label],[X|Univ],[[]|Inst],AllLits,AllLits,
              UnExp,Sleep,AllLab,NonGrd,Free,Forbid).


% The diamond rule

prove(dia Fml,Ind,Label,Univ,Inst,_,AllLits,UnExp,Sleep,AllLab,
      NonGrd,Free,Forbid) :- !,
	reverse([((Fml,Ind,Z),Fml)|Label],TmpNewLabel),
        append(TmpNewLabel,_,NewLabel),
        prove(NonGrd,Ind,_,_,_,AllLits,AllLits,
              [(Univ:Inst:[((Fml,Ind,Z),Fml)|Label]:Fml:Ind)|UnExp],
              Sleep,[NewLabel|AllLab],[],Free,Forbid).


% Check old closures

prove([(L1,L2,RevLabel)|RestNonG],Ind,_,_,_,_,AllLits,UnExp,Sleep,
	...
	( L1 = L2,
	  ...
	  check_forbid(Forbid),
          !
	; prove(RestNonG,Ind,_,_,_,AllLits,AllLits,UnExp,Sleep,[NewLabel|AllLab],
                NonGrd,Free,Forbid)
        ).
...
prove([],_,_,_,_,_,AllLits,[(Univ:Inst:Label:Fml:Ind)|UnExp],Sleep,AllLab,
      NonGrd,Free,Forbid) :- !,
        prove(Fml,Ind,Label,Univ,Inst,AllLits,AllLits,UnExp,Sleep,AllLab,
              NonGrd,Free,Forbid).
...
prove(Lit,_,LitLabel,_,_,[],AllLits,UnExp,Sleep,AllLab,NonGrd,Free,Forbid) :- !,
        ( (Lit = -Neg; -Lit = Neg) ->
          ( UnExp = [(Univ:Inst:Label:Fml:Ind)|UnExpR] ->
	    prove(Fml,Ind,Label,Univ,Inst,
                  [(LitLabel:Neg)|AllLits],[(LitLabel:Neg)|AllLits],
                  UnExpR,Sleep,AllLab,NonGrd,Free,Forbid)
          ; wake_up(Sleep,(Univ:Inst:Label:Fml:Ind),SleepR),
	    prove(Fml,Ind,Label,Univ,Inst,
                  [(LitLabel:Neg)|AllLits],[(LitLabel:Neg)|AllLits],
...
prove(Lit1,Ind, Label1,_,_,[(Label2:Lit2)|Lits],AllLits,UnExp,Sleep,
      AllLab,NonGrd,Free,Forbid) :-
	( \+(Label1:Lit1 = Label2:Lit2) ->
 	  prove(Lit1,Ind,Label1,_,_,Lits,AllLits,UnExp,Sleep,AllLab,NonGrd,
                Free,Forbid)
        ; copy_term((Label1,Free),(NewLabel1,Free)),
          ...
          ( NewLabel1 = NewLabel2,
            ...
            check_forbid(Forbid),
            !
          ; prove(Lit1,Ind,Label1,_,_,Lits,AllLits,UnExp,Sleep,AllLab,
