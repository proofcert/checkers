...
decidem_ke (lmfs-cert (dectree I D) M1 M2 Bnd (state _ M4)) I _ (lmfs-cert (dectree I D) M1 M2 Bnd (state [] M4')) :-
...
storem_kc (lmfs-cert DT M1 M2 Bnd (state [H|T] M4)) H (lmfs-cert DT M1 M2 Bnd (state T [decide-bound-entry H Bnd | M4])).

releasem_ke C C.

initialm_ke (lmfs-cert (dectree I _) _ (init-map M2) _ _) O :- member (init-entry I O) M2.

orNegm_kc (lmfs-cert (dectree I [D]) M1 M2 Bnd (state [] M4)) (lmfs-cert D M1 M2 Bnd (state [lind I, rind I] M4)).

andNegm_kc (lmfs-cert (dectree I [D1,D2]) M1 M2 Bnd (state [] M4))
