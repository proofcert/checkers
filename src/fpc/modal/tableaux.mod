...
decide_ke (modtab-cert (dectree I D) M1 M2 (state _ M3 M4)) I (modtab-cert (dectree I D) M1 M2 (state [] M3 M4')) :-
    reduce_or_set_decide_bound M4 I (s _) M4'.

% this fails if it doesnt find an entry with value greater than z
reduce_or_set_decide_bound [decide-bound-entry I (s B)| R] I (s B) [decide-bound-entry I B| R]. % reduce if found
reduce_or_set_decide_bound [X|L] I B [X|L'] :- reduce_or_set_decide_bound L I B L'.

store_kc (modtab-cert DT M1 M2 (state [H|T] M3 M4)) _ H (modtab-cert DT M1 M2 (state T M3 [decide-bound-entry H (s (s z))|M4])).
...
orNeg_kc (modtab-cert (dectree I [D]) M1 M2 (state [] M3 M4)) _ (modtab-cert D M1 M2 (state [lind I, rind I] M3 M4)).
% orNeg generated by the translation of the theorem
orNeg_kc (modtab-cert D M1 M2 (state [S] M3 M4)) ((n (rel _ _) !-! _)) (modtab-cert D M1 M2 (state [relind,S] M3 M4)). % can orNeg appear above andPos? I want andPos to reset the tree and set the leaf node index to relind
orNeg_kc (modtab-cert D M1 M2 (state [S] M3 M4)) ((p (rel _ _) !-! _)) (modtab-cert D M1 M2 (state [relind,S] M3 M4)). % can orNeg appear above andPos? I want andPos to reset the tree and set the leaf node index to relind
% since we never use again the decision tree. Check with Marco.

andNeg_kc (modtab-cert (dectree I [D1,D2]) M1 M2 (state [] M3 M4)) _
  (modtab-cert D1 M1 M2 (state [lind I] M3 M4)) (modtab-cert D2 M1 M2 (state [rind I] M3 M4)).
...
all_kc (modtab-cert (dectree I [S]) M1 M2 (state [] M3 M4))
  (Eigen\ modtab-cert S M1 M2 (state [lind I] [eigen-entry I Eigen|M3] M4)).
% for extensions of K, we will need to define also a case where the first list of S is not []

some_ke (modtab-cert (dectree I [S]) (diabox-map M1) M2 (state [] M3 M4)) Eigen
  (modtab-cert S (diabox-map M1) M2 (state [bind I O] M3 M4)) :-
