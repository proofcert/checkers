...
orNeg_kc (modtab-cert D M1 M2 M5 Bnd (state [] M3 M4)) ((some (x\ (n (rel x x)))) !-! F) (modtab-cert D M1 M2 M5 Bnd (state [AxInd] M3 M4)) :-
  !, mapsto AxInd (some (x\ (n (rel x x)))).
orNeg_kc (modtab-cert D M1 M2 M5 Bnd (state [] M3 M4)) ((some (x\ some (y\ some z\ ((p (rel x y) &+& p (rel y z)) &+& n (rel x z) ) ))) !-! F) (modtab-cert D M1 M2 M5 Bnd (state [AxInd] M3 M4)) :-
